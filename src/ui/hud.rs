...
pub fn draw_hud(hero: &Hero, assets: &Assets) {
    ...
    // Draw background
    draw_rectangle(0.0, hud_y, screen_w, hud_height, Color::new(0.15, 0.15, 0.15, 1.0));
    ...
    draw_text(&format!("HP: {}/{}", hero.stats.current_hp, hero.stats.max_hp), stats_x, stats_y + 25.0, 20.0, RED);
    draw_text(&format!("MP: {}/{}", hero.stats.current_mp, hero.stats.max_mp), stats_x, stats_y + 50.0, 20.0, BLUE);
    draw_text(&format!("Str: {}  Agi: {}  Int: {}", hero.stats.strength, hero.stats.agility, hero.stats.intelligence), stats_x, stats_y + 75.0, 20.0, WHITE);
